...
class LineSensor_c {
  ...
  public:
    ...
    void DigitalLineFollowingLeft() {
      ...
      LeftPWM = constrain( LeftPWM, -MaxTurnPWM, MaxTurnPWM);
      RightPWM = constrain( RightPWM, -MaxTurnPWM, MaxTurnPWM);
    ...
    unsigned long ReadLeftmostSensor() {
      ...
      //      pinMode( which_pin, OUTPUT );
      //      digitalWrite( which_pin, HIGH );
      ChargePin( LS_LEFTMOST_PIN );
      delayMicroseconds( 10 );
      DischargePin( LS_LEFTMOST_PIN );

      //      pinMode( which_pin, INPUT);
      unsigned long start_time = micros();
      unsigned long break_time = 5000;

      while ( digitalRead( LS_LEFTMOST_PIN ) == HIGH ) {
        // Do nothing here (waiting).
        unsigned long end_time = micros();
        unsigned long elapsed_time = end_time - start_time;
        if (elapsed_time > break_time) {
    ...
    unsigned long ReadMiddleSensor() {
      ...
      while ( digitalRead( LS_MIDDLE_PIN ) == HIGH ) {
        // Do nothing here (waiting).
        unsigned long end_time = micros();
        unsigned long elapsed_time = end_time - start_time;
        if (elapsed_time > break_time) {

...
const float ROBOT_RADIUS_TUNED = 0.73F; // distance from the centre of robot to wheel 4.5F in cm, 6.85 old value
...
const float DISTANCE_PER_COUNT_MM = 0.28057763F;
...
class Kinematics_c {
  public:
    ...
    struct Distance find_distance(volatile long count_left, volatile long count_right) {
      ...
      distance.theta_r_der = (distance.left - distance.right) / (2 * ROBOT_RADIUS_TUNED);
      return distance;
    }
    ...
    void update(volatile long count_left, volatile long count_right) {
      ...
      dist_to_home = sqrt( pow(X_pos, 2) + pow(Y_pos, 2) );
      angle = atan2(-Y_pos, -X_pos) - theta;

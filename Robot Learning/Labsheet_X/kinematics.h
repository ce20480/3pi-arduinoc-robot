...
const float ROBOT_RADIUS_CM = 7.2F; // distance from the centre of robot to wheel 4.5F in cm, 6.85 old value
...
//const unsigned long PERIOD = 50; //every 5 seconds
...
class Kinematics_c {
  public:
    ...
    //      distance.left = (count_left - count_left_pre);
    //      distance.right = (count_right - count_right_pre);
    //      distance.X_r_der = (distance.left + distance.right) / 2;
    //      distance.theta_r_der = (distance.left - distance.right) / (2 * ROBOT_RADIUS_MM);
    //      return distance;
    //    }
    ...
    //    struct Distance find_distance_simple_test(volatile long count_left, volatile long count_right) {
    //      struct Distance distance;
    //      distance.left = (count_left - count_left_pre) * ANGULAR_DISPLACEMENT_PER_COUNT;
    //      distance.right = (count_right - count_right_pre) * ANGULAR_DISPLACEMENT_PER_COUNT;
    //      distance.X_r_der = ((ROBOT_RADIUS * distance.left) + (ROBOT_RADIUS * distance.right)) / 2;
    //      distance.theta_r_der = ((ROBOT_RADIUS * distance.left) / (2 * ROBOT_RADIUS)) - ((ROBOT_RADIUS * distance.right) / (2 * ROBOT_RADIUS));
    //      return distance;
    //    }

    //    struct Distance find_distance_advanced(volatile long count_left, volatile long count_right, unsigned long elapsed_time) {
    //      struct Distance distance;
    //      distance.left = ((count_left - count_left_pre) * DISTANCE_PER_COUNT_MM) / elapsed_time;
    //      distance.right = ((count_right - count_right_pre) * DISTANCE_PER_COUNT_MM) / elapsed_time;
    //      distance.X_r_der = (distance.left + distance.right) / 2;
    //      distance.theta_r_der = (distance.left - distance.right) / (2 * ROBOT_RADIUS_CM);
    //      return distance;
    //    }
    //
    //    struct Distance find_distance_advanced_test(volatile long count_left, volatile long count_right) {
    //      struct Distance distance;
    //      distance.left = ((count_left - count_left_pre) / ((CLICKS_PER_ROTATION * GEAR_RATIO) * WHEEL_CIRCUMFERENCE));
    //      distance.right = ((count_right - count_right_pre) / ((CLICKS_PER_ROTATION * GEAR_RATIO) * WHEEL_CIRCUMFERENCE));
    //      distance.X_r_der = (distance.left + distance.right) / 2;
    //      distance.theta_r_der = (distance.left - distance.right) / (2 * ROBOT_RADIUS_MM);
    //      return distance;
    //    }
    ...
    void update(volatile long count_left, volatile long count_right) {
      ...
      //        if (simple) {
      //
      //          distance = find_distance_simple(count_left, count_right);
      //
      //        } else {
      //
      //          distance = find_distance_advanced(count_left, count_right, elapsed_time);
      //          //          distance = find_distance_advanced_test(count_left, count_right);
      //
      //        }
